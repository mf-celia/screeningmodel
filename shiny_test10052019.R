...
server<-shinyServer(function(input, output) {
  output$distPlot <- renderPlot({
   ...
   #**************************************************ONE-WAY SENSITIVITY ANALYSES (OWSA)  END************************************************************************************************
    ...
    #renderText({    
  # renderTable ####This might help us show the frontier and its ICER results
  ...
  output$a <- renderText({
    ...
    a<- matrix(PlotData, nrow=length(strategies), ncol=3, dimnames=list(c(strategies),c("D_Costs","D_Effects","ICER")))
    
    
    a<-as.data.frame(PlotData)
    
    A<-nearPoints(a,input$plot_click,xvar="D_Effects",yvar="D_Costs", threshold = 300, maxpoints = 1)
  ...
  output$info <- renderPrint({  
    ...
    #nearPoints(PlotData,input$plot_click,xvar="D_Effects",yvar="D_Costs",panelvar1="D_Effects",panelvar2="D_Costs",addDist=TRUE)
