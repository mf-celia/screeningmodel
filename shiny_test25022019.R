...
ICERTable			=data.frame(read.table(file="OutputFiles/LatestResults/ICERTable2019-02-28.txt",    			header=T))
...
strategies <- strategies[!(strategies %in% "20_70_1_1")]
  ...
  fluidRow(
    column(3,
           ...
           radioButtons("parameters", label = "Parameters", choices = list(
            ...
            # "Test Sensitivity 2" = "TestSensitivity2" ,                
            # "Test Sensitivity 3" = "TestSensitivity3" ,                 
            # "Test Sensitivity 4" = "TestSensitivity4" ,                
            # "Test Sensitivity 5" = "TestSensitivity5" ,                 
             "Test Specificity" = "TestSpecificity1" ,                
            # "Test Specificity 2" = "TestSpecificity2" ,                 
            # "Test Specificity 3" = "TestSpecificity3" ,                
            # "Test Specificity 4" = "TestSpecificity4" ,                 
            # "Test Specificity 5" = "TestSpecificity5" ,               
    ...
    column(8,offset = 1,
           ...
           sliderInput("startage", label = "Screen Startage", min = 20, max = 70, value=30, step = 5),
...
server<-shinyServer(function(input, output) {
  output$distPlot <- renderPlot({
   ...
   #**************************************************ONE-WAY SENSITIVITY ANALYSES (OWSA)  END************************************************************************************************
      ...
      #source("C:/Users/yilin/Downloads/DARTH workshop/decision modeling using r/Day 1/Functions/CEA_functions.R")
      ### Get CEA frontier 
      #ce.mat <- cbind(Strategy = 1:20, 
      #                Cost = as.numeric(ICERTable[Scenario,][colnames(ICERTable) %in% paste("D_Costs",strategies,sep="_")])/10^6, 
      #                Effectiveness = as.numeric(ICERTable[Scenario,][colnames(ICERTable) %in% paste("D_Effects",strategies,sep="_")]))
      #ce.front <- getFrontier(ce.mat,maxWTP = Threshold, plot = TRUE)
  ...
  output$a <- renderText({
      ...
      paste(nearPoints(PlotData,input$plot_click,xvar="D_Effects",yvar="D_Costs",threshold = 5000,addDist=TRUE))
    })
     
  output$info <- renderPrint({  
         
    paste0("Life-year gained: ", input$plot_click$x, "\nCost:£ ", input$plot_click$y, "  (million)","\nStrategy: ")
    #,rownames(PlotData)[PlotData[,"D_Effects"] %in% paste(input$plot_click$x)]
#,isolate(reactive({as.character(output$a)}))
    #nearPoints(PlotData,input$plot_click,xvar="D_Effects",yvar="D_Costs",panelvar1="D_Effects",panelvar2="D_Costs",threshold = 5000,addDist=TRUE)
  })
})
